...
function computePanelRect(buttonEl: HTMLElement): { top: number; left: number; width: number } {
  const r = buttonEl.getBoundingClientRect();
  const maxW = Math.min(window.innerWidth - VIEWPORT_PAD * 2, PANEL_MAX_W);
  const width = maxW;
  let left = r.left;
...
const CrmInstancePickerInner: React.FC<CrmInstancePickerProps> = ({
  ...
  const selectedKeys = useMemo(
    () => new Set(selected.map((s) => `${s.channel}:${s.id}`)),
    [selected]
  );
  ...
  const panel =
    open && instances.length > 0 && panelRect
      ? createPortal(
          <div
            ref={panelRef}
            className="fixed z-[200] flex max-h-[min(20rem,50vh)] flex-col overflow-hidden rounded-2xl border border-slate-200/90 bg-white shadow-lg dark:border-slate-600 dark:bg-slate-900 [contain:layout_paint] [isolation:isolate] transform-gpu"
            style={{
              top: panelRect.top,
              left: panelRect.left,
          ...
          >
            <div className="shrink-0 border-b border-slate-100 bg-slate-50/95 px-3 py-2.5 dark:border-slate-700 dark:bg-slate-800/95">
              <p className="text-xs font-medium text-slate-600 dark:text-slate-400">{t('crm.instancesPanelHint')}</p>
              <div className="mt-2 flex flex-wrap gap-2">
                <button
                  type="button"
                  onClick={() => onSelectAll()}
                  className="rounded-lg bg-white px-2.5 py-1 text-xs font-semibold text-sky-600 shadow-sm ring-1 ring-sky-200/80 transition-colors hover:bg-sky-50 dark:bg-slate-800 dark:text-sky-400 dark:ring-sky-500/30 dark:hover:bg-slate-700"
                >
                ...
                <button
                  type="button"
                  onClick={() => onClear()}
                  className="rounded-lg bg-white px-2.5 py-1 text-xs font-semibold text-slate-600 shadow-sm ring-1 ring-slate-200 transition-colors hover:bg-slate-50 dark:bg-slate-800 dark:text-slate-400 dark:ring-slate-600 dark:hover:bg-slate-700"
                >
            ...
            <ul className="min-h-0 flex-1 overflow-y-auto overscroll-contain py-1">
              {instances.map((inst) => {
                const checked = selectedKeys.has(`${inst.channel}:${inst.id}`);
                  ...
                  <li key={`${inst.channel}:${inst.id}`}>
                    <label className="flex cursor-pointer items-center gap-3 px-3 py-2.5 transition-colors hover:bg-slate-50 dark:hover:bg-slate-800/80">
                      <input
                        ...
                        className="h-4 w-4 rounded border-slate-300 text-sky-600 focus:ring-2 focus:ring-sky-500 focus:ring-offset-0 dark:border-slate-500 dark:bg-slate-800"
                      />
                      <span
                        className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-[10px] font-bold ${
                          wa
                            ? 'bg-emerald-500/15 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-300'
                            : 'bg-pink-500/15 text-pink-800 dark:bg-pink-500/20 dark:text-pink-200'
                        }`}
                      >
                        {wa ? 'WA' : 'IG'}
                      </span>
  ...
  return (
    <div className="relative">
      ...
      >
        <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-sky-500/10 ring-1 ring-slate-200/80 dark:bg-sky-400/15 dark:ring-slate-600/60">
        ...
        {count > 0 && (
          <span className="flex h-7 min-w-[1.75rem] shrink-0 items-center justify-center rounded-full bg-sky-600 px-2 text-xs font-bold tabular-nums text-white shadow-sm dark:bg-sky-500">
            {count}
          </span>
        )}
        <svg
          className={`h-4 w-4 shrink-0 text-slate-400 transition-transform duration-200 ease-out dark:text-slate-500 ${open ? 'rotate-180' : ''}`}
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
          strokeWidth={2}
