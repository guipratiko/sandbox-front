...
// ========== Group Message Templates & Scheduled Messages (Mensagens de grupos) ==========
export type GroupMessageType = 'text' | 'media' | 'poll' | 'contact' | 'location' | 'audio';
export type GroupMessageTargetType = 'all' | 'specific' | 'campaign';
...
export const groupMessageTemplatesAPI = {
  getByInstance: async (instanceId: string): Promise<{ status: string; data: GroupMessageTemplate[] }> => {
    return request<{ status: string; data: GroupMessageTemplate[] }>(
      `/groups/message-templates?instanceId=${encodeURIComponent(instanceId)}`
    );
  },
  ...
  create: async (data: {
    ...
    return request<{ status: string; data: GroupMessageTemplate }>('/groups/message-templates', {
      method: 'POST',
      body: JSON.stringify(data),
    });
  },
  ...
  update: async (
    id: string,
    data: { name?: string; description?: string | null; contentJson?: Record<string, unknown> }
  ): Promise<{ status: string; data: GroupMessageTemplate }> => {
    return request<{ status: string; data: GroupMessageTemplate }>(`/groups/message-templates/${id}`, {
      method: 'PUT',
      body: JSON.stringify(data),
    });
  },
  ...
  delete: async (id: string): Promise<{ status: string }> => {
    return request<{ status: string }>(`/groups/message-templates/${id}`, { method: 'DELETE' });
  },
};
...
export const groupMessagesAPI = {
  ...
  getScheduled: async (instanceId: string): Promise<{ status: string; data: GroupScheduledMessage[] }> => {
    return request<{ status: string; data: GroupScheduledMessage[] }>(
      `/groups/messages/scheduled?instanceId=${encodeURIComponent(instanceId)}`
    );
  },

  cancelScheduled: async (id: string): Promise<{ status: string }> => {
    return request<{ status: string }>(`/groups/messages/scheduled/${id}/cancel`, { method: 'POST' });
  },
};
